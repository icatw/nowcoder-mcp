...
@mcp.tool(description="Analyze recurring interview topics from Nowcoder discuss search results. Read-only.")
def analyze_nowcoder_interview_topics(
    query: str,
    max_pages: int = 1,
    max_posts: int = 5,
    use_auth: bool = False,
    ...
    try:
        return to_jsonable(
            client.analyze_interview_topics(
                query=query,
                max_pages=max_pages,
                max_posts=max_posts,
                use_auth=use_auth,
